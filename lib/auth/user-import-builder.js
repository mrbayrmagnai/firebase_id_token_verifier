...
function populateUploadAccountUser(user, userValidator) {
    ...
    if (typeof user.passwordHash !== 'undefined') {
        ...
        result.passwordHash = toWebSafeBase64(user.passwordHash);
    }
    if (typeof user.passwordSalt !== 'undefined') {
        if (!validator.isBuffer(user.passwordSalt)) {
            throw new error_1.FirebaseAuthError(error_1.AuthClientErrorCode.INVALID_PASSWORD_SALT);
        }
        result.salt = toWebSafeBase64(user.passwordSalt);
    }
        ...
        switch (options.hash.algorithm) {
            ...
            case 'HMAC_MD5':
                ...
                populatedOptions = {
                    ...
                    signerKey: toWebSafeBase64(options.hash.key),
                };
                break;
            case 'MD5':
            case 'SHA1':
            ...
            case 'SCRYPT': {
                ...
                populatedOptions = {
                    ...
                    saltSeparator: toWebSafeBase64(options.hash.saltSeparator || Buffer.from('')),
                };
                break;
            }
            case 'BCRYPT':
